                ...
                if (restaurant == null) {
                    ...
                    if (endpoint.startsWith("/restaurants")) {
                        ...
                        if (!exchange.getRequestMethod().equalsIgnoreCase("POST")) {
                            System.out.println("[PROXYHANDLER] l'endpoint /api/bd/reserver doit être appelé en POST");
                            envoyerRequete(exchange, "méthode POST uniquement", 400);
                        }
                        ...
                        String id = JsonBody.getString("idRestaurant");
                        String date = JsonBody.getString("date");
                        String nbpersonne = JsonBody.getString("nbPersonnes");
                        String nom = JsonBody.getString("nom");
                        String prenom = JsonBody.getString("prenom");
                        String telephone = JsonBody.getString("telephone");

                        if (id == null || date == null || nbpersonne == null || nom == null || prenom == null || telephone == null ) {
                            System.out.println("[PROXYHANDLER] l'un des parametre est manquant");
                            envoyerRequete(exchange, "parametre manquant", 400);
                        } else {
                            try {
                                int idRestaurant = Integer.parseInt(id);
                                LocalDateTime dateTime = LocalDateTime.of(date);
                                int nbPersonnes = Integer.parseInt(nbpersonne);

                                String jsonBd = restaurant.reserverTable(idRestaurant, dateTime, nbPersonnes, nom, prenom, telephone);
                                envoyerRequete(exchange, jsonBd, true);
                            } catch (Exception e) {
                                System.out.println("[PROXYHANDLER] erreur de conversion : " + e.getMessage());
                                envoyerRequete(exchange, e.getMessage(), 400);
                            }
